---
description: Load the most recent session file from ~/.claude/save-sessions/ and resume work with full context from where the last session ended.
---
...
```
/resume-session                                                      # loads most recent file in ~/.claude/save-sessions/
/resume-session 2024-01-15                                           # loads most recent session for that date
/resume-session ~/.claude/save-sessions/2024-01-15-session.tmp           # loads a specific legacy-format file
/resume-session ~/.claude/save-sessions/2024-01-15-abc123de-session.tmp  # loads a current short-id session file
```
...
1. Check `~/.claude/save-sessions/`
2. Pick the most recently modified `*-session.tmp` file
3. If the folder does not exist or has no matching files, tell the user:
   ```
   No session files found in ~/.claude/save-sessions/
...
- If it looks like a date (`YYYY-MM-DD`), search `~/.claude/save-sessions/` for files matching
